...
namespace OpenLDR.Dashboard.API.Controllers
{
    ...
    public class GeneralController : ControllerBase
    {
        ...
        //Example = https://[Domain]:[Port]/api/openldr/general/e98389ca62d99875ba7a4e0f2929960b/v1/json/orglevel
        [HttpGet("{apikey}/v{version}/{returntype}/eidtrends")]
        public async Task<string> GetEIDTrends(string apikey, double version, string returntype, [FromQuery] string province, [FromQuery] string district, [FromQuery] string facility)
        {
            try
            {
                if (version.Equals(ApiVersion))
                {
                    if (!string.IsNullOrEmpty(returntype) && ValidReturnTypes != null && ValidReturnTypes.Contains(returntype.ToLower()))
                    {
                        if (!string.IsNullOrEmpty(apikey) && apikey.Length == 32 && ApiKey == apikey)
                        {
                            ...
                            if (!string.IsNullOrEmpty(ConnectionString))
                            {
                                var EIDTrends = EIDTrend.All(ApiConfiguration, ConnectionString, province, district, facility);

                                var list = new List<EIDTrend>();
                                list.AddRange(EIDTrends);
